    ...
    public class MedicoService {
        ...
        public MedicoService(MedicoRepository medicoRepository, PasswordEncoder passwordEncoder, UsuarioRepository usuarioRepository, ConsultaRepository consultaRepository, HorarioRepository horarioRepository, AuthService authService) {
        ...
        public Page<MedicoResponseDto> getAllMedicos(String especialidade, String cidade, Pageable pageable) {
            log.info("Buscando todos os médicos");
            Page<MedicoResponseDto> response = medicoRepository.findAll(especialidade, cidade, pageable).map(m -> {
        ...
        public List<HorarioDisponivelDto> getHorariosDisponiveis(UUID id, LocalDate data) {
            ...
            Set<LocalDateTime> datasConsultas = consultaRepository.findByMedico_IdAndStatus(id, Status.PENDENTE).stream().map(ConsultaModel::getDataConsulta).collect(Collectors.toSet());
            ...
            List<HorarioDisponivelDto> horariosDisponiveis = horarios.stream().filter(HorarioMedico::isAtivo)
                    .flatMap(horario -> {
                        ...
                        while (horaInicio.isBefore(horario.getHorarioFinal())) {
                            ...
                            if (!datasConsultas.contains(LocalDateTime.of(data, horaInicio))) {
